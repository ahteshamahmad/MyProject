...
import { axiosPrivate, GET_MANAGER_URL } from '../../api/api';
...
const ManagerDetailsPage = () => {
    ...
    useEffect(() => {
        async function fetch() {
            loadingNotification()
            axiosPrivate.get(`${GET_MANAGER_URL}${mid}`)
                .then(res => {
                    console.log(res?.data);
                    setManagerDetails(res?.data);
                    successNotification("Manager Details fetched successfully");
                })
                .catch(err => {
                    console.log(err);
    ...
    const { firstName, middleName, lastName, designation, manager, email, officeLocation, dateOfJoining } = managerDetails;
    return (
        <Container my={0}>
                ...
                <Grid gutter="md" grow>
                    <Grid.Col xs={12}>
                        <Box sx={{ maxWidth: 900 }} mt={20} mx="auto" color='dimmed' className='text-left'>
                            ...
                            <Text align="left" color="dimmed" size="m">{designation || 'NA'}</Text>
                            <SimpleGrid cols={2} my={20}>
                                <Grid>
                                    ...
                                    <Grid.Col span='auto'>
                                        <Text color="violet" size="sm" mt={3} fw={500}>Manager Code</Text>
                                        <Text size="md" fw={400}>{manager?.managerCode || 'NA'}</Text>
                                    </Grid.Col>
                                </Grid>
                                <Grid>
                                    <Grid.Col span={2}>
                ...
                <Grid gutter="md" grow>
                    <Grid.Col xs={12}>
                        <Box sx={{ maxWidth: 900 }} mt={60} mx="auto" color='dimmed' className='text-left'>
                            ...
                            <Text align="left" color="violet" fz={40} fw={700}>{manager?.activeProjects || 0}</Text>
                            {/* <Text align="left" fz={20} fw={500}>Completed Projects</Text> */}
                            {/* <Text align="left" color="violet" fz={40} fw={700}>{manager?.completedProjects || 0}</Text> */}
                        </Box>
                    </Grid.Col>
                </Grid>
            </SimpleGrid>
